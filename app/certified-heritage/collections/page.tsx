...
export default function CertifiedCollectionsPage() {
  const { language } = useLanguage()
  const [items, setItems] = useState<CertifiedHeritageItem[]>([])
  const [error, setError] = useState<string | null>(null)
  const supabase = getSupabaseBrowserClient()
  const title = language === "en" ? "Guided Collections" : "المجموعات الموجهة"

  useEffect(() => {
    async function loadItems() {
      if (!supabase) return
      try {
        const nextItems = await getCertifiedItemsByType(supabase, "collection")
        setItems(nextItems)
      } catch (loadError) {
      ...
      <main className="flex-1">
        ...
        <section className="py-16">
          <div className="container mx-auto space-y-6 px-4">
            ...
            <div className="grid gap-6 md:grid-cols-3">{items.map((item) => <CertifiedContentCard key={item.id} item={item} />)}</div>
          </div>
        </section>
      </main>
      <Footer />
