...
export default function CertifiedDocumentsPage() {
  const { language } = useLanguage()
  const [items, setItems] = useState<CertifiedHeritageItem[]>([])
  const [error, setError] = useState<string | null>(null)
  const supabase = getSupabaseBrowserClient()
  const title = language === "en" ? "Certified Documents" : "الوثائق المعتمدة"

  useEffect(() => {
    async function loadItems() {
      if (!supabase) return
      try {
        const nextItems = await getCertifiedItemsByType(supabase, "document")
        setItems(nextItems)
      } catch (loadError) {
